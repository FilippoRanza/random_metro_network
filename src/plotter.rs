...
impl<'a> Plotter<'a> {
    ...
    pub fn draw_points<T: Localizable>(&mut self, points: &[T], symbol: Option<char>) {
        let x = points.iter().map(Localizable::get_x);
        let y = points.iter().map(Localizable::get_y);

        self.ax
            .points(x, y, &[PointSize(2.), PointSymbol(symbol.unwrap_or('S'))]);
    }

    pub fn draw_line<T: Localizable>(&mut self, p1: &T, p2: &T) {
        let (x1, y1) = p1.coordinates();
        let (x2, y2) = p2.coordinates();
        self.ax.lines([x1, x2], [y1, y2], &[Color("black")]);
    }

    pub fn draw_lines<T: Localizable>(&mut self, points: &[T], adj_mat: &Array2<f64>) {
